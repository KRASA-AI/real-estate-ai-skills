...
Audit a real estate agent as an *entity* across the AI discovery layer — not a listing page, not a single bio, but the cross-surface footprint that determines whether ChatGPT, Perplexity, Google AI Overviews, Claude, Gemini, Bing Copilot, or the **portal-resident AI assistants** (Realtor.com's RealAssist on Gemini, Zillow's AI) will name this agent when a buyer or seller asks "who's the best agent for [my situation] in [my market]?" The portal-resident surface is the 2026 addition: buyers increasingly ask the assistant *inside* Zillow or Realtor.com — and that assistant answers from the portal's own structured agent-profile fields (specialty tags, review density, transaction count), not from the open web, so an agent can rank well across the six standalone engines and still be invisible to the assistant a buyer actually opens first. Produces an **Off-Page Discoverability Score** (six audit surfaces × six AI engines), a **Content-Engine Taste Score** (five-dimension anti-slop rubric for AI-assisted personal-brand content), a **Specialization Ladder placement** (five tiers from Generalist → Luxury), a **Brand-Voice + Voice-Clone Asset Inventory** (three-tier discipline shared with `listing-video-workflow.md`), a **6-check compliance sweep**, and a **90-day Action Plan** sequenced across three phases (Stabilize 0–30, Differentiate 30–60, Compound 60–90). The skill is tool-agnostic — it governs the methodology decisions before HeyGen, ElevenLabs, Regen IO, Luxury Presence, Lofty, Sierra Interactive, or any vendor production.
...
**Quick Start — minimum viable run.** Provide the 5 Required Core inputs (agent identity, current digital footprint, specialization claim, jurisdiction, agent config) and the skill produces a defensible Pass-1 audit: Off-Page Discoverability Score, Per-Engine Tactics matrix, Specialization Ladder placement, 6-Check Compliance Sweep, and a 90-Day Action Plan. Sections that depend on the 5 enrichment inputs (Content-Engine Taste Score, Brand-Voice + Voice-Clone Inventory, Competitive Context overlay, Trust-Signal density layer, Time-Budget calibration) ship with documented defaults — see Required Input Section B for the per-item fallback. A solo agent can run the audit in one sitting without coordinating with brokerage marketing, the videographer, or the brokerage compliance officer first; re-runs add inputs as they arrive.
...
4. **Jurisdiction** — State (for advertising-rule overlays), and whether the agent works in any of the AI-disclosure-strict jurisdictions (CA AB 723, IL HB 4762, UT SB 149, CO SB 26-189 effective 1/1/2027, TX HB 149, FL SB 442, etc.). The compliance sweep adapts.
5. **Agent config** — `config.yml` provides brokerage name, state, license #, Equal Housing disclaimer, CAN-SPAM footer, and preferred caption voice. Auto-loaded.

### Section B — Required for Full Audit (5 items, each with per-item fallback)

6. **Content-engine inventory** — Which AI tools the agent currently uses for content, branded by name (HeyGen, ElevenLabs, Regen IO, Synthesia, Pictory, Descript, Opus, Castmagic, Canva, ChatGPT, Claude, Gemini, Perplexity, RPR, Restb.ai, Rechat, Lofty, Sierra Interactive, kvCORE, Follow Up Boss, etc.). Note voice-clone use (yes/no/unsure), AI avatar use (yes/no), AI-syndication tool use (yes/no). *Default if omitted:* Content-Engine Taste Score (Step 5) ships with a "[DATA GAP — content-engine inventory not supplied; re-run with tool list to populate D1–D5]" placeholder; D5 cadence-honesty is computed from publicly visible post frequency only.
7. **Trust signals available** — Press mentions (publication, date, URL), podcast appearances (host, date, URL), listicle inclusions (publication, year — REAL Trends "America's Best," Hour Detroit "Top Agent," local "Best Of" awards), reviews count + average per portal (Zillow, Realtor.com, GBP, Yelp), backlinks count (if known via Ahrefs/SEMrush/Moz), and any earned-media artifacts (op-eds, expert quotes, conference talks). *Default if omitted:* Phase 2 of the 90-day plan inserts a single placeholder "audit your trust-signal density at Day 14 — pull listicle inclusion list, Zillow/Realtor/GBP review counts, and one named publication you could pursue" action; Perplexity tactic in Step 3 reads "trust-signal inventory pending."
8. **Competitive context** — 2–3 named local competitors the agent loses to in AI-engine recommendation queries (the agent runs the prompt "Who is the best [specialty] agent in [metro]?" across ChatGPT and Perplexity and reports who got named). *Default if omitted:* the Recommendation Query Baseline (Step 1) ships with a 2-engine minimum (ChatGPT + Perplexity) instead of the 4-engine target and a documented "[DATA GAP — Google AI Overviews and Claude not yet baselined; add to Day-30 re-run]" — competitor naming is captured from whatever queries did run.
9. **Constraints** — Brokerage branding rules (eXp, Compass, Side, KW, Anywhere all have specific bio + photo + license-display requirements); state advertising rules (CA, TX, NY, FL most-restrictive jurisdictions); time budget the agent commits to the 90-day plan (low: < 2 hrs/wk; medium: 2–5 hrs/wk; high: 5+ hrs/wk). *Default if omitted:* skill defaults to brokerage-name template from `config.yml`, medium time budget (3 hrs/wk), and the most restrictive of the agent's jurisdiction set for advertising rules; flags the assumption at the top of the 90-Day Action Plan.
10. **Voice-clone consent + brokerage policy** — Has the agent recorded a voice clone (ElevenLabs, HeyGen, Resemble, Play.ht, Descript Overdub)? Is there a vendor "do-not-train" clause in place? Does the brokerage permit AI voice or avatar use? *Default if omitted:* defaults to the most conservative tier (L0/L1 permitted, L2/L3 require compliance-officer review); Brand-Voice + Voice-Clone Inventory (Step 6) flags every L2/L3 recommendation as "[VERIFY brokerage permits before adopting]."

**Pass model:** A solo agent typically runs Pass 1 (Section A only) on Day 0 to get the 90-Day Action Plan moving, then layers Section B inputs as the audit surfaces what's missing. A brokerage onboarding a cohort typically supplies Section A + Section B simultaneously and runs full audits in batch.
...
1. **Run the Recommendation Query Baseline.** Have the agent run these four queries in ChatGPT (with web search enabled), Perplexity, Google AI Overviews (via google.com search bar), and Claude (with web search), then paste the responses verbatim:
   ...
   **Solo-agent shortcut (Pass 1 default):** if the agent does not have time to run all 16 query-engine cells (4 queries × 4 engines), run the **2×2 minimum**: queries 1 and 2 across ChatGPT + Perplexity (these two engines surface the highest share of buyer-side recommendation traffic per FlyDragon 2026 Benchmark). The audit ships the Recommendation Query Baseline table with the 12 unrun cells flagged "[DATA GAP — re-baseline at Day 30 across all 16 cells]." This is the typical Pass-1 entry path. The full 16-cell baseline is then captured on the Day-30 re-run inside Phase 1.

   **Portal-resident assistant check (add a 5th column where available):** in addition to the four standalone engines, run query 1 ("Who is the best [specialization] agent in [metro]?") inside the agent-facing assistant on Realtor.com (RealAssist) and Zillow, where the buyer's market exposes one. Record whether the portal assistant named the agent and which profile fields it cited. This matters because the portal assistant draws on the portal's *own* structured agent profile — the same S2/S3/S4 surfaces this audit scores in Step 2 — rather than the open web, so a weak Zillow/Realtor.com/Homes.com profile is simultaneously an off-page-discoverability gap *and* a portal-resident-AI gap. If the portal assistant is not available in the agent's market, flag `[PORTAL AI — not yet live in market; monitor]`.
...
2. **Off-Page Discoverability Audit — six surfaces.** For each surface, note presence (Y/N), entity completeness (license # / brokerage / years active / transaction count / designations / specialization / `sameAs` outbound links), reviews count, and last-updated date if visible. Score each surface 0–10:
   ...
   - **S4 — Homes.com profile:** claimed; bio + transaction count + reviews. (Per RISMedia 4/25/2026 reporting that Homes.com agent subscriptions are growing fast, this surface is moving from "optional" to "table stakes" in 2026.) *S2–S4 profile data is MLS-fed; if the agent (or their team) uses an AI tool to bulk-derive or restructure these fields from an MLS export rather than editing the portal UI directly, confirm the MLS's AI-use policy permits it — see `knowledge-base/tools-ecosystem/mls-ai-data-governance.md`.*
...
3. **Per-Engine Discoverability Tactics ledger.** Cross-tabulate the six audit surfaces against the six AI engines from `listing-aeo-optimizer.md` (ChatGPT, Perplexity, Google AI Overviews, Claude, Gemini, Bing Copilot). For each engine, name the two highest-leverage surfaces and the one specific tactic per engine:
   ...
   - **Portal-resident AI (Realtor.com RealAssist on Gemini / Zillow AI):** answers from the portal's *own* structured agent-profile fields, not the open web; prefers Zillow (S2) + Realtor.com (S3) + Homes.com (S4) field completeness and review recency. Tactic = fill every structured profile field the portal exposes (specialty tags incl. the exact specialization phrase, transaction count, markets-served zip list, designations) and keep review recency high (respond within 7 days) — a fact that lives only in free-text bio prose but not in a structured field is invisible to the portal assistant, so flag any specialization or trust signal that is prose-only as `[FIELD GAP — promote into a structured profile field]`. This engine's levers are the same S2/S3/S4 surface scores from Step 2, which is why a portal-profile gap is double-counted: off-page *and* portal-AI.
   The output is a 7×2 matrix (engine → top two surfaces + one tactic). This becomes the targeting layer of the 90-day plan.

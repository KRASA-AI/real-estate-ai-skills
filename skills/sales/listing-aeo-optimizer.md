...
**Minimum viable run (3 inputs):** paste (1) the **source content** (the page copy), (2) the **page type** (`listing-detail` / `neighborhood-guide` / `agent-bio` / `process-article` / `market-report`), and (3) the **primary entities** for that page type (for a listing: address, beds/baths/sqft, price, year built, zip, school district, MLS #). That is enough to ship a **Pass 1 Fast Optimize** — an entity-dense AEO Lede, a default 6-question Q&A block, a Key Facts table, the page-type JSON-LD stack, and an AEO Readiness Score. Everything else in Required Input is **Pass 2 Enrichment** with a stated default, so a solo agent is never blocked on trust-signal inventories or site-level `llms.txt` audits before getting usable copy. Run Pass 2 when the page is a flagship listing, an agent-bio page being optimized to win local-agent recommendation queries, or any page where the discoverability surfaces in Step 8 (portal-resident AI + Google home-search) are the priority.

## Purpose

Rewrite and restructure a listing page, neighborhood guide, agent bio, market report, or process article so it is the source that answer engines cite when buyers and sellers ask questions — not just a page that ranks in classic blue-link search. "Answer engines" in mid-2026 means three distinct surfaces, not one: (1) **standalone LLMs** (ChatGPT, Perplexity, Claude, Gemini, Bing Copilot); (2) **AI overviews inside classic search** (Google AI Overviews / AI Mode); and (3) — new and rising fast — **portal- and platform-resident AI agents** (Realtor.com's RealAssist built on Google Gemini, Zillow's AI-mode search, and Google's national home-search / listing-ads surface fed by HouseCanary). A listing now has to be legible to a Gemini agent living *inside* Realtor.com or Google Home Search, not only to a buyer who opens ChatGPT in a separate tab. The output is a drop-in content block with an entity-dense AEO Lede, an explicit Q&A surface, a Key Facts table, JSON-LD structured-data blocks, an Authoritative Sources footnote stack, an Entity Glossary, an `llms.txt` snippet for crawler guidance, an engine-specific tactics ledger (per surface: ChatGPT / Perplexity / Google AI Overviews / Claude / Gemini / Bing Copilot / **portal-resident AI** / **Google home-search**), an 8-check fair-housing + advertising compliance sweep, and an AEO Readiness Score on a five-dimension rubric (Entity Density / Answerability / Citation Density / Structured Data / Author Authority).

The skill is anchored in two April 2026 benchmarks: the 5WPR × Haute Residence "2026 Luxury Real Estate AI Discovery Report" (real-estate AI Overview trigger rate = 0.14%, the lowest of any tracked vertical, indicating that specific entity-rich pages disproportionately win the few citations that exist) and the FlyDragon 2026 Benchmark Report (91% of agents are invisible to AI assistants; LLM citation-position distribution favors position-1 at 44.2%, position-2 at 31.1%, position-3 at 24.7%). Both reinforce the same finding: AEO is a small-pool game where entity density, named-source citation, and structured data are the durable wins. A 2026 carrier-signal cluster sharpens *where* that small pool now lives: Google's I/O conversational-Search overhaul (June 2026) and the AI-Overview position-1 CTR collapse (Ahrefs: 7.3% → 1.6% on AI-Overview queries) pushed discovery off blue links; Realtor.com's RealAssist (Gemini-based, June 2026) and Zillow's AI search moved buyer discovery *inside the portal*; and Google's home-search pilot moved to a **national listing-ads rollout** (June 2026, HouseCanary-fed). The durable wins are the same — entity density, named-source citation, structured data — but the page now has to win them on a portal-resident agent's terms too. A standing sensitivity note from the same cluster: LLM property-valuation output is inconsistent and position-dependent (an LLM has given a buyer and a seller opposite values on the same home), so never let an answer-engine-optimized page imply an LLM-sourced valuation is authoritative — cite the MLS comp set and the human agent's pricing judgment.
...
The skill runs in two passes. **Pass 1 = Fast Optimize** (Required Core — produces a shippable, entity-dense optimized block). **Pass 2 = Enrichment** (refines the page against the trust-signal, site-level, and discoverability-surface inputs). Each Pass 2 item carries a **Default if omitted** so a missing input never blocks the run.

### Pass 1 — Required Core (3 inputs)
...
4. **Buyer/seller questions to capture** — 3–6 real questions the agent expects buyers or sellers to type into an AI chat about this subject. *Default if omitted:* infer the standard 6-question set for the page type from the taxonomies in Step 3.
5. **Trust signals available** — Credentials, years of experience, transaction count, awards, press mentions, named cited datasets (MLS, Census, GreatSchools, Walk Score, NOAA, FEMA, district report card, parcel record, BLS, BEA, FHFA HPI). *Default if omitted:* use only `config.yml` license + brokerage and flag Author Authority as the dimension to enrich.
6. **Target AI surfaces** — Which answer engines the agent cares most about. *Default if omitted:* optimize for all eight surfaces in Step 8 (ChatGPT, Perplexity, Google AI Overviews, Claude, Gemini, Bing Copilot, portal-resident AI, Google home-search).
7. **Existing schema/JSON-LD status** — Is structured data already on the page? (yes / no / partial / unknown). If partial, paste the existing JSON-LD so the skill avoids duplication. *Default if omitted:* assume `none` and produce the full page-type stack, flagged "verify no duplication before publishing."
8. **Site-level signals** — Current `llms.txt` content if present; sitemap URL; canonical-URL pattern; server-side vs. client-side rendering. *Default if omitted:* assume no `llms.txt` (generate one) and flag client-side-render crawlability as an unverified P2.
9. **Jurisdiction** — State (for fair-housing and advertising rules carried over to AEO copy) and MLS (some MLSs require specific listing-detail-page disclaimers that must survive the AEO rewrite). *Default if omitted:* pull state from `config.yml`; run the full U.S. compliance sweep; flag MLS-specific disclaimers as "verify against your MLS."
...
- **Determine the pass.** If only the three Required Core inputs are present, run **Pass 1 Fast Optimize** using the Pass 2 defaults above; label the output `Pass 1 — Fast Optimize` and list the applied defaults at the top so a fast draft is never mistaken for a fully enriched page. If any Pass 2 inputs are present, run **Pass 2 Enrichment** on those and default the rest. Either way the full Step 1–12 process runs; the pass only governs which inputs are real vs. defaulted.
...
8. **Write a per-engine tactics ledger.** For each target AI surface, produce one paragraph describing the engine-specific behavior the page should be optimized against. The differences are real and persistent enough to matter:
   ...
   - **Portal-resident AI (Realtor.com RealAssist on Gemini, Zillow AI search):** The fastest-growing 2026 surface — a Gemini- (or proprietary-) based agent that lives *inside the portal* and answers a buyer's natural-language question ("which of these has a real yard and a sub-15-minute commute?") against the portal's own structured feed, not against your website's HTML. You do not optimize the portal's UI; you optimize **the listing record you feed the portal**. Tactic: make the structured fields the portal ingests (MLS/IDX feed, RESO-standard fields, the public-remarks/agent-remarks split) carry the same entity density and source-cited facts as the AEO Lede — the portal agent answers from the field, so a fact absent from the structured feed is invisible to it even if it's in your prose. Put the school name + rating + source-date, the $/sqft, the HOA, and the renovation year into the *fields*, not only the description. Keep public remarks free of fair-housing proxies (the portal agent will surface them verbatim). Flag any fact that lives only in unstructured prose as a `[FEED GAP]`. A growing number of MLSs now restrict AI processing of their data under the data-license agreement (see `knowledge-base/tools-ecosystem/mls-ai-data-governance.md`) — if the agent's own AI tool (not the portal's) is being used to derive or restructure these fields from an MLS export, confirm that use is permitted before running this step.
   - **Google home-search / national listing-ads surface (HouseCanary-fed):** Google's home-search moved from an 8-metro pilot (May 2026) to a **national listing-ads rollout** (June 2026), surfacing listings inside the AI-Overview-shaped search UX with HouseCanary as a data partner. Tactic: this surface rewards the same structured-data parity as Google AI Overviews (Step C8) plus a clean, complete `RealEstateListing` JSON-LD block with `price`, `floorSize`, `numberOfBedrooms`, `address`, and `dateModified` all present and matching the visible page; ensure the listing is syndicated to the feeds Google's partner ingests and that the canonical URL is stable. A listing with stale or partial structured data is the one that loses the national-surface slot to a competitor's complete record.
...
- **Portal-resident AI (Realtor.com RealAssist / Zillow AI):** `[FEED GAP]` flagged — the GreatSchools 8/10 rating, the $470/sqft, and the 2023 kitchen-renovation year are in the prose but not confirmed in the IDX structured fields. Fix: push school name + rating + source-date, $/sqft, and renovation year into the MLS/RESO fields so the portal's Gemini agent can answer "is this in a strong school zone and recently renovated?" from the feed. Public remarks confirmed free of fair-housing proxies.
- **Google home-search (national listing-ads surface):** `RealEstateListing` JSON-LD complete (price, floorSize, bedrooms, address, dateModified all present and page-visible); canonical URL stable; verify syndication reaches the HouseCanary-fed feed. Meets the national-surface structured-data bar.

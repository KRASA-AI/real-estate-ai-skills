...
3. **Agent capacity** — How many listing conversations the agent can realistically start in the next 30 days (Pass 1) or sustain across the next 90 days (Pass 2). Governs how deep down the ranked list to go. *Default if omitted: `config.yml → capacity.listing_conversations_per_month`; if config is silent, default to 20/month and flag the assumption on the Pipeline Summary — capacity drives where the ranked list is cut, so a defaulted capacity is stated, never silent.*
4. **Outreach channels allowed** — Phone, SMS, email, handwritten note, door-knock, direct mail, social DM (constrained by TCPA and CAN-SPAM rules). *Default if omitted: the channels the agent actually operates, from `config.yml → tools.crm` / `tools.marketing` / `voice.followup_style`. Config may only **narrow** the channel set, never widen it: a channel the agent has not opted into is not recommended, and no config value can authorize an SMS to a contact without opt-in or a call to a DNC-listed number.*
5. **Protected sensitivities** — Any contact flagged do-not-contact, in active distress, in litigation, asked for space, or under any DNC / CAN-SPAM / two-party-consent restriction. *No default — an unsupplied sensitivity list is treated as "not yet screened," and every top-band contact carries a `[SENSITIVITY UNSCREENED]` flag until the agent confirms. Absence of input is never read as absence of restriction.*
...
7. **Market context** — Current median DOM and MOI band per `market-analysis-summary.md` four-band ladder (< 3 / 3–5 / 5–8 / > 8), YoY price appreciation in the area, average seller-concession percentage. *Default if omitted: read the **cached ZIP market profile** produced by `market-analysis-summary.md` for each contact's ZIP — the same shared profile `lead-qualification-bant.md` consumes — rather than re-deriving the market per contact. Stamp the Market-Fit component with the profile's refresh date; if the profile is more than 60 days stale or absent for a ZIP, score Market-Fit at the band midpoint and flag `[MARKET DATA STALE — E component low confidence]`.*
...
11. **Agent goal context** — Listings closed YTD, target listings count for year, brokerage convention for seller-pipeline KPIs, the agent's preferred CMA template. *Default if omitted: pull the annual listing target and KPI convention from `config.yml → goals` (the same values `annual-business-plan-builder.md` writes), and the CMA template from `config.yml → tools.cma`.*
12. **Agent config** — `config.yml` is the personalization backbone for this skill, not just a signature block. It supplies: **capacity** (input #3 default), **operating channels** (input #4 default, narrowing-only), **service area** (which ZIP profiles to load for the Market-Fit component), **goals** (input #11 default), **`tools.cma`** (which CMA template the hand-off names), **`voice.tone` / `always_use` / `never_use`** (the register of every generated outreach angle in Step 6 — `never_use` terms are blocked outright), **brokerage + state + license** (the compliance overlay and any written-addendum rule on MM price-reduction calls), the **two-party-consent state list**, and any agent-specific TCPA registration. Auto-loaded. **Config never loosens a guard:** it cannot authorize a channel the compliance rules forbid, cannot downgrade a `[FRAUD-SCREEN]` or sensitivity flag, and a missing config field is flagged `[ADD TO CONFIG]` rather than invented.
...
- Load `config.yml` **first** and resolve it into the run's defaults before touching the contact list: capacity (#3), operating channels (#4, narrowing-only), service-area ZIP profiles for Market-Fit (#7), goals/KPI convention and CMA template (#11), outreach voice (`voice.tone` / `always_use` / `never_use`), brokerage + state + license, two-party-consent list, TCPA registration. State every defaulted value at the top of the Pipeline Summary so the agent can see what to override — a capacity or channel default silently applied changes where the list is cut and how the agent is told to reach people.
- Verify the Step-2 weight table before scoring: **every stage column must sum to exactly 5.00.** If a column does not, stop and correct it — do not score against a broken column (see Step 2 for why this silently caps a stage's top band).
...
3. **Compute the five score components per contact.**
   ...
   **B. Tenure Score (0–20)** — Median US tenure at sale is 13 years; local medians vary. Band: <3 years → 4 (sunk costs, low intent unless life event); 3–5 years → 10; 5–8 years → 14; 8–13 years → 18; 13+ years → 20. For move-up markets or starter-home zips, compress: the 5–8 year band becomes the peak. For Mid-Marketing stage, tenure becomes near-irrelevant (component weighted **0.25** in MM — the Step-2 table is canonical).
   ...
   **D. Behavioral Score (0–20)** — Points for observed actions, capped at 20: website activity on home-value or sold-listing pages last 30 days +8; opened 3+ agent emails last 60 days +4; clicked CMA or home-value link +10; requested home valuation +15; asked about contractor referrals or repair bids +6; liked or commented on sold-listing posts +3. For Mid-Marketing stage, the component shifts to listing-specific signals: showings/feedback responsiveness +6; price-reduction discussion engagement +8; willingness to refresh photos / restage +6 (component weighted **1.5** in MM stage — the Step-2 table is canonical). For Post-Pending re-engagement, behavioral signals include reason-for-fall-out responsiveness and willingness to relist after seasoning (weighted **1.5** in PP stage).

   **E. Market-Fit Score (0–20)** — Does the CURRENT market favor selling this specific home? Points: supply shortage in zip (MOI < 3) +12; price-per-sqft above purchase by 40%+ +6; days-on-market in zip < 21 +6; rate trend favorable (rates dropping > 50 bps last 90 days) +4; seller-concession percentage in zip < 2% +2. Cap at 20. Subtract 4 if MOI > 6 or DOM > 60. The MOI band is the canonical input from `market-analysis-summary.md` four-band ladder — prefer the **cached ZIP market profile** that skill emits (the same profile `lead-qualification-bant.md` consumes) over a re-derived read, and use its refresh date as the confidence stamp on this component. For Mid-Marketing stage, this component rises to a **2.0** weight (the Step-2 table is canonical) — the question becomes "given current market, can this listing reasonably sell at the current price?"
...
- **The Step-2 weight table is canonical.** Any weight cited in prose elsewhere in this skill defers to it, and every stage column must sum to exactly 5.00. A column that sums low silently caps that stage's maximum score below 100 and makes its own top band unreachable — verify before scoring, never after.
- **Config personalizes; it never permits.** `config.yml` may narrow the recommended channel set, set the voice, and supply capacity/goal/market defaults — it may not authorize a channel the compliance rules forbid, override a DNC/TCPA/two-party-consent suppression, or downgrade a `[FRAUD-SCREEN]` or sensitivity flag. A missing config field is flagged `[ADD TO CONFIG]`, never invented.
